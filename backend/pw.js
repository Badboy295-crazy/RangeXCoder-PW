...
module.exports = function(wss) {
    ...
    async function getPWVideoData(batchId, subjectId, scheduleId, tag, subjectSlug, chapterSlug, hostUrl = 'https://rangexcoder-backend.onrender.com') {
        ...
        try {
            ...
            // If we still don't have it, assume scheduleId itself might be the videoId
            if (!videoDetailsId) {
            ...
            if (videoDetailsId) {
                ...
                if (rawVideoUrl) {
                    ...
                    if (uuidMatch) {
                        ...
                        videoUrl = `${hostUrl}/stream-proxy/${uuid}/master.m3u8?token=${encodeURIComponent(streamToken)}`;
        ...
        // Find current lecture and compile lists
        ...
        return {
            ...
            scheduleId: safeEncrypt(scheduleId),
            rawBatchId: batchId,
    ...
    router.get(['/api/batches', '/api/batche'], async (req, res) => {
        try {
    ...
    router.get('/api/todays-schedule', async (req, res) => {
        const { batchId } = req.query;
        const decryptedBatchId = safeDecrypt(batchId);
        try {
    ...
    router.get('/api/dpp-tests', async (req, res) => {
    ...
    router.get('/api/get-video-url', async (req, res) => {
    ...
    router.get('/api/video-timeline', async (req, res) => {
    ...
    router.get('/api/decrypt-token', (req, res) => {
            ...
            if (decryptedUrl.includes("tap=video")) {
                try {
                    ...
                    if (batchId && subjectId && scheduleId) {
                        const hostUrl = req.protocol + '://' + req.get('host');
                        const data = await getPWVideoData(batchId, subjectId, scheduleId, tag, null, null, hostUrl);
    ...
    router.get('/get-dpp-quiz', async (req, res) => {
        const { token } = req.query;
        let targetUrl = "";
    ...
    router.get('/api/video-data', async (req, res) => {
        const { token } = req.query;
        if (!token) {
        ...
        try {
            ...
            const hostUrl = req.protocol + '://' + req.get('host');
            const data = await getPWVideoData(batchId, subjectId, scheduleId, tag, subjectSlug, chapterSlug, hostUrl);
    ...
    router.get('/get-test-solution-video', async (req, res) => {
        const { token } = req.query;
        let targetUrl = "";
    ...
    router.get(/^\/stream-proxy\/([a-zA-Z0-9_-]+)\/(.+)$/, async (req, res) => {
    ...
    router.get('/:uuid/hls-key', async (req, res) => {
    ...
    router.post('/api/posts/like', (req, res) => {
    ...
    router.post('/api/posts/comment', (req, res) => {
    ...
    router.post('/api/admin/delete-post', (req, res) => {
