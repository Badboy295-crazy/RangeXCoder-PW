...
app.get('/api/proxy/content-details', verifyRequest, async (req, res) => {
    try {
        ...
        const response = await fetch(urls.detailUrl);
        res.json(await response.json());
    } catch(e) { res.status(500).json({ success: false, message: e.message }); }
});

app.get('/api/proxy/batches', verifyRequest, async (req, res) => {
    try {
        const source = req.query.source || 'nt_api_2';
        const urls = getApiUrls(source, "null");
        const response = await fetch(urls.batchesUrl);
        res.json(await response.json());
    } catch(e) { res.status(500).json({ success: false, message: e.message }); }
});

app.get('/api/proxy/course-details', verifyRequest, async (req, res) => {
    try {
        ...
        const response = await fetch(urls.courseDetailsUrl);
        res.json(await response.json());
    } catch(e) { res.status(500).json({ success: false, message: e.message }); }
});

app.get('/api/proxy/all-content', verifyRequest, async (req, res) => {
    try {
...
app.get('/', (req, res) => {
    res.send("🚀 RangeXCoder Secure Proxy & Dual-Sync Engine is LIVE!");
});
...
app.listen(PORT, () => {
    console.log(`Server Keep-Alive Port running on ${PORT}`);
});
...
async function fetchAPI(url) {
    try { 
        ...
        console.error("Fetch API Error for URL:", url, e.message);
        return null; 
    }
}
...
// Uses isSyncing flag to prevent overlap — no double-sync even on rapid cron ticks
...
if (!cronScheduled) {
    ...
    console.log("🚀 Server Deep Sync Engine Scheduled for Every 1 Minute...");
}
